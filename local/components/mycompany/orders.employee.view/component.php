...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"] === "N"
 ...
 'VALUE' => '01.' . $i . '.' . date('Y')
    ...
    $arResult['ITEMS'] =
        ORDERS\OrdersManager::GetOrdersEmployee($arParams["USER_ID"]);


    foreach($arResult['ITEMS'] as &$arOrders) {
        $arValue = ORDERS\OrdersManager::GetOrdersEmployeeValue($arOrders["ID"], $arParams["USER_ID"], $_REQUEST['UF_PERIOD']);
        $arOrders["ORDERS_ID"] = $arValue["ID"];
        $arOrders["ORDERS_ID_VALUE"] = $arValue["UF_VALUE"];
    }
